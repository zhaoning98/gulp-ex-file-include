    ...
    function includeHandler(inst) {
      ...
      if (args) {
        ...
        if (args[3]) {
          ...
          // include data in inherited variable
          else if (args[3].trim().match(/^`.*`$/)) {
            let key = args[3].trim().replace(/`/g, '')
            obj[key] = data[key]
          }
          else {
            // include data in the function
            try {
        ...
        recFile = include(recFile, includeContent, obj)
